    ...
    public class Polygon : Surface
	{
		...
		public override bool IsPointInside(PointF point)
		{
			try
			{
				...
				var vertex1 = mVertice[mVertice.Length - 1];
				PointF vertex2;

				for (var i = 0; i < mVertice.Length; ++i)
				{
					vertex2 = mVertice[i];
						...
						((vertex2.Y <= point.Y) && (point.Y < vertex1.Y)))
					{
						...
						(point.Y == vertex1.Y) && (point.Y == vertex2.Y) && ((point.X < vertex1.X) || (point.X < vertex2.X))
					)
                    {
		...
		public override bool IsRectangleIntersect(RectangleF rectangle)
		{
			...
			for (i = 0; i < mVertice.Length; i++)
			{
				if (
					(mVertice[i].X > rectangle.Left)
					&& (mVertice[i].X < rectangle.Right)
					&& (mVertice[i].Y > rectangle.Top)
					&& (mVertice[i].Y < rectangle.Bottom)
				)
				{
					return true;
				}
			...
			var corner1 = new PointF(rectangle.Left, rectangle.Top);
			var corner2 = new PointF(rectangle.Right, rectangle.Top);
			var corner3 = new PointF(rectangle.Right, rectangle.Bottom);
			var corner4 = new PointF(rectangle.Left, rectangle.Bottom);
			...
			for (i = 1; i < mVertice.Length; i++)
			{
				if (
					AreLinesCrossing(corner1, corner2, mVertice[i - 1], mVertice[i])
					|| AreLinesCrossing(corner2, corner3, mVertice[i - 1], mVertice[i])
					|| AreLinesCrossing(corner3, corner4, mVertice[i - 1], mVertice[i])
					|| AreLinesCrossing(corner4, corner1, mVertice[i - 1], mVertice[i])
				)
				{
					return true;
				}
		...
		private bool AreLinesCrossing(PointF line1Point1, PointF line1Point2, PointF line2Point1, PointF line2Point2)
		{
			...
			var vector1 = new PointF(line1Point2.X - line1Point1.X, line1Point2.Y - line1Point1.Y);
			var vector2 = new PointF(line2Point2.X - line2Point1.X, line2Point2.Y - line2Point1.Y);

			// compute a common divider and check if it's not null
			var divider = (-vector2.X * vector1.Y) + (vector1.X * vector2.Y);
			if (divider == 0.0f)
            {
				return false;
            }

			var vector3 = new PointF(line1Point1.X - line2Point1.X, line1Point1.Y - line2Point1.Y);
			var s = ((-vector1.Y * vector3.X) + (vector1.X * vector3.Y)) / divider;
			var t = ((vector2.X * vector3.Y) - (vector2.Y * vector3.X)) / divider;
