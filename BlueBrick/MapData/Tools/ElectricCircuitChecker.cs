...
namespace BlueBrick.MapData.Tools
{
    ...
    class ElectricCircuitChecker
	{
		...
		{
			if (startingBrick == null || startingBrick.ElectricCircuitIndexList == null)
            {
				return;
            }
		...
		private static void CheckFromOneBrick(LayerBrick.Brick startingBrick)
		{
			...
			var shortcuts = new List<LayerBrick.Brick.ConnectionPoint>();
			...
			var firstConnection = startingBrick.ConnectionPoints[startingBrick.ElectricCircuitIndexList[0].mIndex1];
			...
			while (mBricksToExplore.Count > 0)
			{
				...
				var needToReexploreTheBrick = false;
				...
				foreach (var circuit in brick.ElectricCircuitIndexList)
				{
					// get the connection point of the current circuit inside the brick
					var start = brick.ConnectionPoints[circuit.mIndex1];
					var end = brick.ConnectionPoints[circuit.mIndex2];
					...
					var connectionLink = end.ConnectionLink;
			...
			foreach (var connection in shortcuts)
			{
